...
from services.create_update_product_from_biz_to_main import CreateUpdateProductFromBizToMain
...
class ProcessSyncOrderService():
    ...
    def run(self):
        ...
        elif self.store == "CreateUpdate to Main":
            handle = self.sync_orders.get('handle', None)
            if handle is not None:
                CreateUpdateProductFromBizToMain([handle]).run()
